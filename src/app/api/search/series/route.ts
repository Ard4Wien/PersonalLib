...
export async function GET(request: Request) {
    try {
        ...
        if (!userId) {
            console.error("Dizi arama hatası: Kullanıcı oturumu bulunamadı (Yetkisiz)");
            return NextResponse.json({ error: "Yetkisiz: Lütfen giriş yapın" }, { status: 401 });
        }

        if (!TMDB_API_KEY) {
            console.error("Dizi arama hatası: TMDB_API_KEY eksik");
            return NextResponse.json({ error: "TMDb API anahtarı yapılandırılmamış (Vercel ayarlarını kontrol edin)" }, { status: 500 });
        }
